...
class BuildLLVMTriton:
    ...
    @property
    def llvm_targz(self) -> pathlib.Path:
        """Path to tar bundle"""
    ...
    def cmake_builddir(self) -> pathlib.Path:
        ...
        return self.buildroot / f"build-{self.llvm_commit}"
    ...
    def unpack(self, targz: pathlib.Path) -> None:
        ...
        if llvm_project_root.is_dir():
            logger.info("Removing old src dir %s", llvm_project_root)
            shutil.rmtree(llvm_project_root)
        logger.info("Unpacking %s", targz)

        # we need license, cmake, and project dirs
        root = llvm_project_root.name
        parts = "|".join(["LICENSE.TXT", "cmake", *self.enable_projects])
        pattern = re.compile(f"^{root}/({parts})(/.*)?$")
    ...
    def cmake_configure(self) -> None:
        ...
        cmake_builddir = self.cmake_builddir
        if cmake_builddir.is_dir():
            logger.info("Removing old cmake build dir %s", cmake_builddir)
            shutil.rmtree(cmake_builddir)
    ...
    def pack_build(self) -> pathlib.Path:
        ...
        tarbundle = self.buildroot / f"llvm-triton-{self.llvm_shortcommit}.tar"
    ...
    def podman_import(self, tarbundle: pathlib.Path) -> str:
        ...
        tag = f"{self.container_registry}/{self.llvm_triton_name}-{os_id}{os_version_id}:latest"
        ...
        logger.info("Importing %s into container %s", tarbundle, tag)
        cmd = [
            "podman",
            "import",
            str(tarbundle),
        ...
        labels = {
            ...
            "org.opencontainers.image.description": "LLVM for Triton with MLIR",
    ...
    def build(self) -> str:
        ...
        for directory in [self.llvm_project_root, self.destdir, self.cmake_builddir]:
            if directory.is_dir():
                logger.info("rmtree %s", directory)
                shutil.rmtree(directory)
        if self.container_registry:
            return self.podman_import(tarbundle)
        else:
            return tarbundle.name
